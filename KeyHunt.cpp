...
void KeyHunt::output(std::string addr, std::string pAddr, std::string pAddrHex, std::string pubKey) {
  ...
  if (!needToClose)
    printf("\n");
  fprintf(f, "Address: %s\n", addr.c_str());
  std::string prv = pAddrHex;
  if (prv.length() < 64) {
    prv = std::string(64 - prv.length(), '0') + prv;
  }
...
std::string addr = secp->GetAddress(h0);
...
ITEM it = found[i];
if (coinType == COIN_BTC) {
std::string addr = secp->GetAddress(it.hash);
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
double CalcPercantage(Int& ICount, Int& rangeStart, Int& rangeDiff2)
{
    if (rangeDiff2.IsZero())
        return 100.0;

    // Work in double by extracting 64-bit approximations
    // Convert big Int to double via base-16 string
    std::string countHex  = ICount.GetBase16();
    std::string diffHex   = rangeDiff2.GetBase16();

    // strtoull handles up to 64-bit; for larger ranges use leading digits
    // We compare ratio: ICount / rangeDiff2 * 100
    // To avoid overflow, shift both right by same number of bits if > 64 bit
    int diffLen  = (int)diffHex.size();
    int countLen = (int)countHex.size();

    // Pad to equal length for fair comparison
    // Use up to 15 hex digits (60 bits) for double precision
    const int MAX_HEX = 15;
    std::string dStr = diffHex.size()  > MAX_HEX ? diffHex.substr(0,  MAX_HEX) : diffHex;
    std::string cStr = countHex.size() > MAX_HEX ? countHex.substr(0, MAX_HEX) : countHex;

    // Scale count if it came from fewer hex digits (i.e. it's smaller)
    int diffScale  = (int)diffHex.size()  - (int)dStr.size();
    int countScale = (int)countHex.size() - (int)cStr.size();

    double dDiff  = (double)strtoull(dStr.c_str(),  nullptr, 16);
    double dCount = (double)strtoull(cStr.c_str(),  nullptr, 16);

    // Adjust for truncated digits: multiply by 16^(dropped digits)
    for (int i = 0; i < diffScale;  i++) dDiff  *= 16.0;
    for (int i = 0; i < countScale; i++) dCount *= 16.0;

    if (dDiff <= 0.0) return 100.0;

    double perc = (dCount / dDiff) * 100.0;
    return perc;
}
// ----------------------------------------------------------------------------
